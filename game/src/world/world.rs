...
use crate::{RuntimeFactory, TaskManager};
...
impl World {
    pub fn new(task_manager: TaskManager, attachments: Vec<Box<dyn WorldAttachment + Send>>) -> Self {
    ...
    pub fn generate(world_data: &mut WorldData) -> Vec<Room> {
        todo!()
    }
    ...
    pub async fn update_async(mut world_data: WorldData, mut attachments: Vec<Box<dyn WorldAttachment + Send>>) {
